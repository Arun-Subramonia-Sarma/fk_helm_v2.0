...
{{- $businessLabels := include "generic-chart.businessLabels" . }}
{{- if $businessLabels }}
{{ $businessLabels }}
{{- end }}
{{- end }}
...
labels:
  ...
  {{- if and $workloadLabels (ne $workloadLabels "") }}
...
{{- if and $workloadAnnotations (ne $workloadAnnotations "") }}
annotations:
...
{{/* Also check workload-specific environment variables and merge them */}}
{{- if $workloadConfig.env }}
  {{- $existingEnv := $containerConfig.env | default list }}
  {{- $_ := set $containerConfig "env" (concat $existingEnv $workloadConfig.env) }}
{{- end }}
{{- if $workloadConfig.envVars }}
  {{- $existingEnvVars := $containerConfig.envVars | default dict }}
  {{- range $key, $value := $workloadConfig.envVars }}
    {{- $_ := set $existingEnvVars $key $value }}
  {{- end }}
  {{- $_ := set $containerConfig "envVars" $existingEnvVars }}
{{- end }}
{{- if $workloadConfig.envValueFrom }}
  {{- $existingEnvValueFrom := $containerConfig.envValueFrom | default dict }}
  {{- range $key, $value := $workloadConfig.envValueFrom }}
    {{- $_ := set $existingEnvValueFrom $key $value }}
  {{- end }}
  {{- $_ := set $containerConfig "envValueFrom" $existingEnvValueFrom }}
{{- end }}
{{- if $workloadConfig.envFrom }}
  {{- $existingEnvFrom := $containerConfig.envFrom | default list }}
  {{- $_ := set $containerConfig "envFrom" (concat $existingEnvFrom $workloadConfig.envFrom) }}
{{- end }}
...
spec:
  ...
  containers:
    - name: {{ $containerConfig.name | default $globalConfig.Chart.Name }}
      ...
      {{- $envVars := list }}
      {{- $envMap := dict }}
      {{/* Add global environment variables from env array */}}
      {{- if $globalConfig.Values.env }}
        {{- range $globalConfig.Values.env }}
          {{- $_ := set $envMap .name . }}
        {{- end }}
      {{- end }}
      {{/* Add container-specific environment variables from env array */}}
      {{- if $containerConfig.env }}
        {{- range $containerConfig.env }}
          {{- $_ := set $envMap .name . }}
        {{- end }}
      {{- end }}
      {{/* Convert envVars key-value pairs to Kubernetes env format */}}
      {{- if $globalConfig.Values.envVars }}
        {{- range $key, $value := $globalConfig.Values.envVars }}
          {{- $_ := set $envMap $key (dict "name" $key "value" ($value | toString)) }}
        {{- end }}
      {{- end }}
      {{- if $containerConfig.envVars }}
        {{- range $key, $value := $containerConfig.envVars }}
        ...
        {{- end }}
      {{- end }}
      {{/* Add envValueFrom environment variables */}}
      {{- if $globalConfig.Values.envValueFrom }}
        {{- range $key, $valueFrom := $globalConfig.Values.envValueFrom }}
          {{- $_ := set $envMap $key (dict "name" $key "valueFrom" $valueFrom) }}
        {{- end }}
      {{- end }}
      {{- if $containerConfig.envValueFrom }}
        {{- range $key, $valueFrom := $containerConfig.envValueFrom }}
      ...
      {{- if $globalConfig.Values.envFrom }}
        {{- range $globalConfig.Values.envFrom }}
          {{- if .secretRef }}
            {{- $_ := set $envFromMap .secretRef.name . }}
          {{- else if .configMapRef }}
            {{- $_ := set $envFromMap .configMapRef.name . }}
          {{- end }}
        {{- end }}
      {{- end }}
      {{- if $containerConfig.envFrom }}
        {{- range $containerConfig.envFrom }}
          {{- if .secretRef }}
            {{- $_ := set $envFromMap .secretRef.name . }}
          {{- else if .configMapRef }}
            {{- $_ := set $envFromMap .configMapRef.name . }}
      ...
      {{- end }}
      {{/* Convert map back to list */}}
      {{- range $key, $envFromVar := $envFromMap }}
        {{- $envFromVars = append $envFromVars $envFromVar }}
      {{- end }}
      {{- if $envFromVars }}
      envFrom:
